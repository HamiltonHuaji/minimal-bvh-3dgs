...
inline aabb_type merge(const aabb_type& lhs, const aabb_type& rhs) noexcept
{
    return aabb_type{
        .lower = float3{
            .x = fminf(lhs.lower.x, rhs.lower.x),
            .y = fminf(lhs.lower.y, rhs.lower.y),
            .z = fminf(lhs.lower.z, rhs.lower.z),
        },
        .upper = float3{
            .x = fmaxf(lhs.upper.x, rhs.upper.x),
            .y = fmaxf(lhs.upper.y, rhs.upper.y),
            .z = fmaxf(lhs.upper.z, rhs.upper.z),
        },
    };
}
