...
__device__ inline int common_upper_bits(const uint64_t lhs, const uint64_t rhs) noexcept
{
    return ::__clzll(lhs ^ rhs);
}

__device__ __host__ inline std::uint32_t morton_code(float3 xyz, float resolution = 1024.0f) noexcept
{
...
struct morton_code_calculator
{
    ...
    __device__ __host__ inline uint32_t operator()(const aabb_type &box) noexcept
    {
        auto p = centroid(box);
        p.x -= whole.lower.x;
        p.y -= whole.lower.y;
...
__device__ inline int32_t find_split(uint64_t const *node_code, const int32_t num_leaves,
    ...
    uint64_t *morton)
{
    ...
    default_aabb.upper.x = -100000.f;
    default_aabb.lower.x = 100000.f;
    default_aabb.upper.y = -100000.f;
    default_aabb.lower.y = 100000.f;
    default_aabb.upper.z = -100000.f;
    default_aabb.lower.z = 100000.f;
    thrust::device_ptr<aabb_type> aabbs_ptr = thrust::device_pointer_cast(aabbs_internal);
    // const auto aabb_whole = thrust::reduce(aabbs_ptr + num_internal_nodes, aabbs_ptr + num_nodes, default_aabb, merge);
    const auto aabb_whole = thrust::reduce(aabbs_ptr + num_internal_nodes, aabbs_ptr + num_nodes, default_aabb, [] __host__ __device__ (const aabb_type& lhs, const aabb_type& rhs) { return merge(lhs, rhs); });
