...
trace_bvh_cuda(int32_t num_rays, int32_t *nodes, float *aabbs,
    ...
    thrust::for_each(thrust::device,
                     ...
                     [nodes, aabbs_internal, rays_o, rays_d, num_contributes,
                      ...
                      means3D, covs3D, position_list] __device__(uint32_t idx) {
                         if (num_contributes[idx] == 0)
                             return;
                ...
                if (node[4] <= 1) { // 8
                    ...
                    while (!stack_device2.empty()) {
                        ...
                        if (node2[4] > 1) {
                            ...
                            if (opacities[object_id2] < 1.f / 255.f)
                                continue;
                            float3 normal = normals[object_id2];
                            if (normal.x * ray_d.x + normal.y * ray_d.y + normal.z * ray_d.z > 0)
                                continue;
